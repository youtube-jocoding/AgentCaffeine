...
**덮개를 닫아도** Mac을 깨어 있게 하고, 작업이 끝나거나 앱이 꺼지면 watchdog와 확인 절차로
**원상 복구**합니다.
...
- 🔌 **비밀번호 요청 최소화** — 보통 켤 때 한 번만 필요하며, watchdog 해제가 실패하면 끌 때 한 번 더 요청
...
3. 끝나면 **잠자기 방지 끄기** 또는 앱 종료 → watchdog가 자동 복구하고, 실패 시 관리자 권한으로 직접 해제
...
3. **끄기·종료** — 신호 파일을 남긴 뒤 실제 `SleepDisabled=0`을 확인합니다. 확인에 실패하면 관리자 권한으로 직접 복구합니다.
